use axum::{extract::{FromRef, FromRequestParts}, http::request::Parts, response::IntoResponse};
use axum_extra::extract::CookieJar;

use crate::{Backend, Session, SessionId, CookieSessionBackend};

pub fn get_session_id(cookie_name: &str, parts: &mut Parts) -> Option<SessionId> {
...
pub async fn load_session<B: Backend + CookieSessionBackend>(backend: B, parts: &mut Parts) -> Result<Result<Session<B>, B>, B::Error> {
    if let Some(session_id) = get_session_id(backend.session_cookie_name(), parts) {
        if let Some(fields) = backend.load_session_data(&session_id).await? {
            Ok(Ok(Session::new(backend, session_id, fields.user_id, fields.data)))
        } else {
